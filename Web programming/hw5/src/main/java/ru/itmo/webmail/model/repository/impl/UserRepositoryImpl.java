...
public class UserRepositoryImpl implements UserRepository {
    ...
    @Override
    public User findByLEmail(String email) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
    ...
    @Override
    public User findByEmailOrLogin(String loginOrEmail, String passwordSha) {
        User f = findByLoginAndPasswordSha(loginOrEmail, passwordSha);
        if (f == null) {
            f = findByEmailAndPasswordSha(loginOrEmail, passwordSha);
        }
        return f;
    }
    ...
    @Override
    public int findIdBySecretCode(String secret) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "UPDATE User SET confirmed=TRUE " +
                            "WHERE id=(SELECT userId FROM EmailConfirmation WHERE secret=?)")) {
