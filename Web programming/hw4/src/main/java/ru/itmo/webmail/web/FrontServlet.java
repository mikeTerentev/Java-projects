...
public class FrontServlet extends HttpServlet {
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Method method = findActionMethod(clazz, route.getAction(), new Class[]{});
        if (method == null) {
            throw new NotFoundException();
        }
        Method beforeMethod = findActionMethod(clazz, "before",
        ...
        try {
            ...
            if (Arrays.equals(method.getParameterTypes(), new Class[]{Map.class})) {
            ...
            throw new ServletException(
                    "Can't run page method [clazz=" + clazz + ", method=" + method + "].", e);
        } catch (Exception e) {
            throw new ServletException(
                    "Can't run page method [clazz=" + clazz + ", method=" + method + "].", e);
        }
    ...
    private Method findActionMethod(Class<?> clazz, String name, Class<?>[] params) {
        Method method = null;
        for (Class<?> clz = clazz; method == null && clz != null; clz = clz.getSuperclass()) {
            if (params.length == 0) {
                method = Arrays.stream(clz.getDeclaredMethods())
