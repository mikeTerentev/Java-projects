    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isFile()) {
            ...
            try {
                ...
                o_hash = new String(digest.digest(body));
            } catch (NoSuchAlgorithmException e) {
                e.printStackTrace();
            }
            String hash = "";
            for (int i = 0; i < o_hash.length(); i++) {
                hash += (int) o_hash.charAt(i);
            }
    ...
    private static final class Request {
        ...
        private Request(String request) {
            ...
            try {
                ...
                //System.out.println(firstLineTokens[1]);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            uri = firstLineTokens[1].endsWith(".png") || firstLineTokens[1].endsWith(".html") ? firstLineTokens[1] : firstLineTokens[1] + "/index.html";
            headers = new HashMap<>();
